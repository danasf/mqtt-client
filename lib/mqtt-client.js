...
function MQTTClient(host, port, ident) {
  ...
  */
  ...
  this.port = port || 1883;
  this.id = ident || 'myMQTT' + Math.floor(Math.random() * 100);   // ident should only be alphanum
  this.protocol = "MQIsdp"; // v4 of protocol uses MQTT, but causes some errors on some servers
  this.version = 3;
